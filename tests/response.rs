...
use json_rpc_types::{Error, ErrorCode, Id, Version};
...
fn success_failure_serialize() {
    ...
    assert_eq!(
        serialized,
        r#"{"jsonrpc":"2.0","error":{"code":-32601,"message":"Method not found","data":"text"},"id":1}"#
    );
}
...
fn response_deserialize_should_fail_on_mixing_result_error() {
    ...
    assert_eq!(
        result.to_string(),
        "JSON-RPC Response contains both result and error field at line 1 column 105"
    );
}
...
fn success_failure_deserialize_without_id() {
    let expected = Response::error(Version::V2, create_error(), None);
    let text =
        r#"{"jsonrpc":"2.0","error":{"data":"text","code":-32601,"message":"Method not found"}}"#;
